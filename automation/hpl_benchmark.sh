...
function run_benchmark() {
    ...
    execute "run_full_linpack" ssh hpcuser@${public_ip} "ssh \$(head -n bin/hostlist) 'cd hpl && mpirun -np $mpiRanks -ppn 2 -hostfile \$HOME/bin/hostlist ./xhpl_intel64_static -n $fullLinpack_N -p $fullLinpack_P -q $fullLinpack_Q -nb $fullLinpack_NB'"
