...
function run_benchmark() {
    ...
    execute "run_benchmark" ssh hpcuser@${public_ip} "ssh \$(head -n1 bin/hostlist) 'cd /mnt/resource/scratch/qe/input && mpirun -np $numProcs -ppn $processesPerNode -hostfile \$HOME/bin/hostlist ../pw.x -input pw_1.in'"
